...
- [x] Phase 4: 추가 기능 (완료 - FilterSidebar, 기술스택 필터, BookmarkButton 분리 완료 / URL 동기화 이월)
- [x] Phase 5: 북마크 기능 (완료)
- [x] Phase 6: 통계/분석 페이지 (완료, 경력 요건 차트·라인 차트 이월)
- [x] Phase 7: 공고 입력 가이드 페이지 (완료)
- [ ] Phase 8: 품질 개선 및 배포 (진행 중)
- [x] Phase 9: 채용공고 자동 수집 파이프라인 (완료)

### Phase 8 세부 체크리스트

**미완성 기능 완성**
- [x] 상세 페이지 북마크 버튼 클라이언트 컴포넌트 분리
- [x] 기술스택 필터 사이드바 추가
- [ ] URL 쿼리 파라미터 필터 동기화
- [ ] Rich Text 어노테이션 렌더링 개선 (bold/italic/code/strikethrough)
- [ ] 경력 요건 분포 차트 추가

**UX/UI 개선**
- [ ] 모바일 필터 드로어 구현
- [x] 공고 카드 스켈레톤 로딩 UI
- [x] 상태 배지 색상 구분 강화 (진행중 초록/마감 회색/검토중 노랑)
